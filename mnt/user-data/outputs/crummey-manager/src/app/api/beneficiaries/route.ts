...
export async function GET(request: NextRequest) {
  ...
  if (!trustId) {
    return NextResponse.json(
      { data: null, error: 'trust_id is required' },
      { status: 400 }
    );
  }

  // Verify user owns the trust
...
export async function POST(request: NextRequest) {
  ...
  if (body.is_minor && (!body.guardian_name || !body.guardian_email)) {
    return NextResponse.json(
      { data: null, error: 'Guardian name and email required for minor beneficiaries' },
      { status: 400 }
    );
  }
  ...
  await supabase.from('audit_log').insert({
    ...
    details: { trust_id: body.trust_id, name: body.full_name },
  });
...
export async function PUT(request: NextRequest) {
  ...
  // Verify user owns the trust via beneficiary
  const { data: existing } = await supabase
    .from('beneficiaries')
    .select('trust_id, trusts!inner(user_id)')
    .eq('id', id)
    .single();

  if (!existing || existing.trusts.user_id !== user.id) {
...
// DELETE /api/beneficiaries - Remove a beneficiary (soft delete via is_active)
